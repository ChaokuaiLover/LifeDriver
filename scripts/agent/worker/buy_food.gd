...
func Update(_delta: float):
	target = worker.buy_target
	conditions = worker.energy < 80 or  worker.buy_food_state == "success" or worker.buy_food_state == "fail" or worker.money < worker.food_price * worker.food_need or target == null or !target.sell_status
